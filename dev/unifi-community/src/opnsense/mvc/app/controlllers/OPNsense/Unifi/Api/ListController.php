...
class ListController extends ApiControllerBase
{
    ...
    # /api/unifi/list/unifi_devices
    {
        $config = Config::getInstance()->object();
        $controlleruser     = $config->OPNsense->unifi->username;
        $controllerpassword = $config->OPNsense->unifi->password;
        $controllerurl      = "https://".$config->OPNsense->unifi->servername.":".$config->OPNsense->unifi->port;
        $controllerversion  = '';
        $site_id            = $config->OPNsense->unifi->siteid;
        $debug              = false;

        $unifi_connection = new UnifiClient($controlleruser, $controllerpassword, $controllerurl, $site_id, $controllerversion);
        $set_debug_mode   = $unifi_connection->set_debug($debug);
        $loginresults     = $unifi_connection->login();
        $aps_array        = $unifi_connection->list_devices();
        return  json_encode($aps_array);
    }

    public function clientsAction()
    # /api/unifi/list/clients
    {
        $config = Config::getInstance()->object();
        $controlleruser     = $config->OPNsense->unifi->username;
        $controllerpassword = $config->OPNsense->unifi->password;
    ...
    # /api/unifi/list/events
    {
        ...
        $set_debug_mode   = $unifi_connection->set_debug($debug);
        $loginresults     = $unifi_connection->login();
        $aps_array        = $unifi_connection->list_events();
        return  json_encode($aps_array);
    }

    public function healthAction()
    # /api/unifi/list/health
    {
        $config = Config::getInstance()->object();
        $controlleruser     = $config->OPNsense->unifi->username;
        $controllerpassword = $config->OPNsense->unifi->password;
        ...
        $set_debug_mode   = $unifi_connection->set_debug($debug);
        $loginresults     = $unifi_connection->login();
        $aps_array        = $unifi_connection->list_health();
        return  json_encode($aps_array);
    }

    public function statusAction()
    # /api/unifi/list/status
    {
        $config = Config::getInstance()->object();
        $controlleruser     = $config->OPNsense->unifi->username;
        $controllerpassword = $config->OPNsense->unifi->password;
        ...
        $set_debug_mode   = $unifi_connection->set_debug($debug);
        $loginresults     = $unifi_connection->login();
        $aps_array        = $unifi_connection->stat_sysinfo();
        return  json_encode($aps_array);
    }

    public function firmwareAction()
    # /api/unifi/list/firmware
    {
        $config = Config::getInstance()->object();
        $controlleruser     = $config->OPNsense->unifi->username;
        $controllerpassword = $config->OPNsense->unifi->password;
        ...
        $set_debug_mode   = $unifi_connection->set_debug($debug);
        $loginresults     = $unifi_connection->login();
        $aps_array        = $unifi_connection->list_firmware();
        return  json_encode($aps_array);
    }

    public function usersAction()
    # /api/unifi/list/statallusers
    {
        $config = Config::getInstance()->object();
        $controlleruser     = $config->OPNsense->unifi->username;
        $controllerpassword = $config->OPNsense->unifi->password;
        ...
        $set_debug_mode   = $unifi_connection->set_debug($debug);
        $loginresults     = $unifi_connection->login();
        $aps_array        = $unifi_connection->list_users();
        return  json_encode($aps_array);
    }
    public function statallusersAction()
    # /api/unifi/list/statallusers
    {
        $config = Config::getInstance()->object();
        $controlleruser     = $config->OPNsense->unifi->username;
        ...
        $set_debug_mode   = $unifi_connection->set_debug($debug);
        $loginresults     = $unifi_connection->login();
        $aps_array        = $unifi_connection->stat_allusers();
        return  json_encode($aps_array);
    }

    public function wlanconfAction()
    # /api/unifi/list/wlanconf
    {
        $config = Config::getInstance()->object();
        $controlleruser     = $config->OPNsense->unifi->username;
        $controllerpassword = $config->OPNsense->unifi->password;
        ...
        $set_debug_mode   = $unifi_connection->set_debug($debug);
        $loginresults     = $unifi_connection->login();
        $aps_array        = $unifi_connection->list_wlanconf();
        return  json_encode($aps_array);
    }

    public function listnetworkconfAction()
    # /api/unifi/list/wlanconf
    {
        $config = Config::getInstance()->object();
        $controlleruser     = $config->OPNsense->unifi->username;
        ...
        $set_debug_mode   = $unifi_connection->set_debug($debug);
        $loginresults     = $unifi_connection->login();
        $aps_array        = $unifi_connection->list_networkconf();
        return  json_encode($aps_array);
    }
